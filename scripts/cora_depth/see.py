...
def get_traj(stag="none", alpha=1.0, depth=4, repeat=0):
    vl = np.load(
        "%s_GraphConv_16_relu_%s_depth_%s_repeat%s/accuracy_vl.npy" % (stag, alpha, depth, repeat)
    )

    te = np.load(
        "%s_GraphConv_16_relu_%s_depth_%s_repeat%s/accuracy_te.npy" % (stag, alpha, depth, repeat)
    )
